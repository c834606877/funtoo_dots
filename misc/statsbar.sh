...
status() {
    cpuld() {
        cpuload=$(cpu)
        echo %{F$CYN}$glycpu$SEP$cpuload$SEP%{F-}
    }
    ...
    echo "%{c}$SEP4$(cpuld)$SEP4$(memory)$SEP4$(overclock)$SEP4$(vol)$SEP4$(clockdate)$SEP4$(clocktime)$SEP4"
}
...
do
  echo "$(status)"
  sleep 2
done | lemonbar -g $notifybar_g -u 3 -d -f "terminus12-8" -f "FontAwesome:size=8" -B $BG -F $WHT
